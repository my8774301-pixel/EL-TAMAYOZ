...
const url = import.meta.env.VITE\_SUPABASE\_URL as string | undefined;
const key = import.meta.env.VITE\_SUPABASE\_ANON\_KEY as string | undefined;
...
  console.warn(
    "[El Tamayoz] Missing VITE\_SUPABASE\_URL / VITE_SUPABASE_PUBLISHABLE_KEY. Copy .env.example to .env and fill them in.",
  );
}

export const supabase = createClient(url ?? "[http://localhost](http://localhost)", key ?? "public-anon-key", {
  auth: {
    persistSession: true,
