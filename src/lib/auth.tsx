...
export function AuthProvider({
  ...
  const emailFor = (username: string) => {
    ...
    if (cleanedUsername === TEACHER_USERNAME_AR) {
      return TEACHER_EMAIL;
    }

    return usernameToEmail(clean(cleanedUsername, 40));
  };

  const signIn: AuthState["signIn"] = async (
    username,
    password
  ) => {
    ...
    if (prof.blocked) {
      ...
      return {
        error:
          "Your account is blocked. Please contact your teacher.",
      };
    }
    ...
    if (prof.role === "student" && !prof.approved) {
      ...
      return {
        error:
          "Your account is waiting for the teacher's approval.",
      };
    }
  ...
  const signUp: AuthState["signUp"] = async (v) => {
    const username = clean(v.username, 40).toLowerCase();
  ...
  const changePassword: AuthState["changePassword"] =
    async (current, next) => {
      const { data: sess } =
  ...
  const changeUsername: AuthState["changeUsername"] =
    async (current, nextUsername) => {
      const { data: sess } =
        await supabase.auth.getSession();
      ...
      const clean_u =
        clean(nextUsername, 40).toLowerCase();
